...
**단일 실행 숫자를 먼저 보지 않는다.** 먼저 볼 것은 이것이다 — `--seed 42`로
**완전히 같은 명령**을 네 번 돌린 결과다.
...
**seed를 고정해도 재현되지 않는다**(MPS 커널의 비결정적 연산 순서). 같은 seed의 편차 0.131이
seed를 바꿔서 생긴 편차(0.108~0.157)와 비슷하다.

seed 3개(42/43/44)로 잰 편차도 두 번 실행하면 이렇게 달라진다.
