...
7. **숫자에는 불확실성을 함께 적는다.** 두 가지가 있고 둘 다 확인해야 한다.
   ...
   - **학습 실행 간 편차**: 학습이 불안정해서 생긴다. `run_seeds.py`로 잰다.
     **seed를 고정해도 재현되지 않으므로**(아래 4절) 여러 번 돌려 범위로 본다.
...
| 지표 | 관측 범위 (7회 학습) |
|---|---|
| 합성 test macro F1 | **0.584 ~ 0.741** |
| attack recall | 0.744 ~ 1.000 |
| benign FPR | 0.275 ~ 0.650 |

**평균이 아니라 범위로 적은 이유가 있다.** `--seed 42`로 완전히 같은 명령을 네 번 돌렸더니
macro F1이 0.584 / 0.625 / 0.637 / 0.715로 나왔다. **seed를 고정해도 재현되지 않는다**
(MPS 커널의 비결정적 연산 순서). 즉 **단일 학습 결과를 소수점 셋째 자리까지 인용하는 것은
근거가 없고, 이 표의 어떤 값도 여러분이 그대로 얻을 값이 아니다.**
